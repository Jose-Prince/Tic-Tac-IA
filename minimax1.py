...
def minimax_sin_poda(tablero, profundidad, es_max, jugador, contador):
    contador[0] += 1
    oponente = 'O' if jugador == 'X' else 'X'
    ...
    if es_max:
        ...
        for move in posibles_movimientos(tablero):
            ...
            eval = minimax_sin_poda(nuevo, profundidad - 1, False, jugador, contador)
        ...
        for move in posibles_movimientos(tablero):
            ...
            eval = minimax_sin_poda(nuevo, profundidad - 1, True, jugador, contador)
            min_eval = min(min_eval, eval)
        return min_eval

def obtener_mejor_movimiento_sin_poda(tablero, jugador, profundidad):
    ...
    for move in posibles_movimientos(tablero):
        ...
        valor = minimax_sin_poda(nuevo_tablero, profundidad - 1, False, jugador, contador)
...
def simular_juego_sin_poda(k, quien_empieza):
    ...
    while evaluar_ganador(tablero) is None and posibles_movimientos(tablero):
        if turno == jugador:
            move, nodos_turno = obtener_mejor_movimiento_sin_poda(tablero, jugador, k)
            if move:
